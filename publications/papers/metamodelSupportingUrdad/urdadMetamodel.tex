...
metamodel'' \cite{_ieee_2003}.

\cite{gonzalez-perez_metamodelling_2008}

\todo{Sort out whether to fo with semantics or modeling features - waiting for response on email sent to authors}

+\emph{AP: I substituted ``modeling features'' for ``semantics''. Please check.} 

The URDAD metamodel provides the semantics required required by the URDAD methodology to store the requirements for a service/use case. for practicality reasons, the semantics has been encoded EMOF/Ecore instead of {\em Web Ontology Language}, OWL\cite{}. It is motivated by the technology support for developing a concrete textual and graphical syntax for the DSL, the support for model-to-model and model-to-text transformations and the availability of OCL as a constraint language for specifying constraints at both, the metamodel and model levels. Mappings between these two technologies can be readily performed \cite{staab_model_2010}. A trasformation to OWL facilitates reasoning services like consistency and completeness assessment.

\subsection{Modeling semantics required by URDAD}
...
section of influences from std modeling languages
