...
URDAD's meta model formalizes the modelling constructs needed for the specification of service requirements 
...
The Object-Constraint Language (OCL) has become de-facto standard for specifying constraints across object graphs. However, in a services-oriented approach and in the context of reusable, parametrised constraints the OCL alone is not expressive enough. In particular, the definition of reusable constraints requires support for binding parameters. 
...
\todo{for Dawid: Query, Assignable and its subclasses have been moved to Process metamodel (does not make sense in static data model) }
...
\todo{For Dawid: Clean up process metamodel diagram after incorporating Query, Assignable }
