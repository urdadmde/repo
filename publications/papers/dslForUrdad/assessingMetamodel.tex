...
Comparing the \emph{model complexities} of the URDAD DSL model for our example to that of the equivalent URDAD UML model, we found that the URDAD DSL model has marginally lower complexity. This is due to the URDAD DSL directly supporting the required semantics for the URDAD methodology, whilst in UML some of the concepts are not directly represented and need to be assembled from more basic model constructs.

The \emph{usage complexity} will depend largely on the diagrammatic syntax tool support. However, it is expected the lower language complexity, the direct representation of core concepts like services contracts and the much lower level of inconsistency risks will contribute to a lower usage complexity.