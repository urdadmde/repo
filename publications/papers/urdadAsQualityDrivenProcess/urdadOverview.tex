...
URDAD\cite{solms_generating_2009} provides a service oriented analysis and design methodology yielding service contracts and process designs for services across levels of granularity. The resultant model represents a \emph{Computation Independent Model} (CIM) as the services contracts and implementing service processes can be realized as either automated system processes or manual processes. Nevertheless, the model has a level of detail and preciseness which is generally available in OMG's \emph{Platform Independent Model} (PIM) and not commonly in CIMs.
